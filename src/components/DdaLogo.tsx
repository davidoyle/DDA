  ...
  const content = (
    <div className={`inline-flex items-end gap-3 ${className}`}>
      ...
      <div className="space-y-1">
        ...
        {!compact ? (
          <>
            <div className="h-[2px] w-full bg-current opacity-40" />
            <p className="text-sm leading-none opacity-70 sm:text-base">public-evidence systems analysis</p>
          </>
        ) : (
          <p className="text-[10px] uppercase tracking-[0.18em] opacity-70">public-evidence systems analysis</p>
        )}
      </div>
    </div>
  );
