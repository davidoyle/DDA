...
function cloneRegister(register: AssumptionRegister): AssumptionRegister {
  return Object.fromEntries(
    Object.entries(register).map(([id, assumption]) => [id, { ...assumption }]),
...
function registerFromState(state: LeverState): AssumptionRegister {
  ...
  const register = cloneRegister(
    scenario && state.scenarioId !== 'custom'
      ? applyScenario(BASE_ASSUMPTIONS, scenario)
      : BASE_ASSUMPTIONS,
  );
...
function scenarioPatch(id: string): Partial<LeverState> {
  ...
  if (typeof scenario.overrides['price.bcPlantInlet.base'] === 'number')
    patch.gasPrice = scenario.overrides['price.bcPlantInlet.base'];
  if (typeof scenario.overrides['infra.pipelineToll'] === 'number')
    patch.pipelineToll = scenario.overrides['infra.pipelineToll'];
...
function buildRoyaltyRows(project: Project, register: AssumptionRegister): DualOutputRow[] {
  ...
  return steps.map((royaltyRate) => {
    const result = buildCashFlow({
      project: { ...project, overrideRoyaltyRate: royaltyRate },
      register,
    });
    return {
      royaltyRate,
...
export default function ModelShell() {
  ...
  const narrative = useMemo(
    () => generateNarrative(result.metrics, dualRows, state.wacc),
    [result.metrics, dualRows, state.wacc],
  );
  const scenarioResults = useMemo(
    () => scenarioResultsFor(selectedProject, state),
    [selectedProject, state],
  );
  const { actualCount, estimatedCount, proxyCount, totalCount } = useMemo(
    () => qualityCounts(register),
    [register],
  );
  const lastCalculated = new Date().toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
    second: '2-digit',
  });
  const govTake =
    result.metrics.governmentTakeUndiscounted /
    Math.max(
      1,
      result.rows.reduce((sum, row) => sum + row.revenue, 0),
    );
  const flagHint =
    estimatedCount > 0 ? `${estimatedCount} estimated inputs active` : 'All assumptions confirmed';
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 print:bg-white">
      <header className="sticky top-0 z-40 flex min-h-14 flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-2 shadow-sm print:static">
        <div className="flex items-center gap-2 font-bold text-[#003366]">
          <span className="flex h-9 w-9 items-center justify-center rounded bg-[#003366] text-white">
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-5 print:max-w-none print:px-0">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            label="Project IRR"
            value={formatIrr(result.metrics.irr)}
            note={flagHint}
            flagsUsed={result.metrics.flagsUsed}
            actualCount={actualCount}
            totalCount={totalCount}
          ...
          <MetricCard
            label="NPV (C$B)"
            value={`$${(result.metrics.npv / 1000).toFixed(1)}B`}
            note={flagHint}
            flagsUsed={result.metrics.flagsUsed}
            actualCount={actualCount}
            totalCount={totalCount}
          ...
          />
          <MetricCard
            label="Provincial Revenue NPV"
            value={`$${(result.metrics.governmentTakeNPV / 1000).toFixed(1)}B`}
            note={flagHint}
            flagsUsed={result.metrics.flagsUsed}
            actualCount={actualCount}
            totalCount={totalCount}
          ...
          />
          <MetricCard
            label="Government Take"
            value={`${(govTake * 100).toFixed(1)}%`}
            note={flagHint}
            flagsUsed={result.metrics.flagsUsed}
            actualCount={actualCount}
            totalCount={totalCount}
        ...
        {mode === 'analyst' ? (
          <AnalystView
            state={state}
            onChange={onPatch}
            dualRows={dualRows.filter(
              (row) => Math.abs(row.royaltyRate - state.royaltyRate) <= 0.151,
            )}
            currentRoyaltyRate={state.royaltyRate}
            rows={result.rows}
            actualCount={actualCount}
      ...
      <footer className="sticky bottom-0 z-30 flex min-h-10 flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-white px-4 py-2 text-xs text-slate-600 print:hidden">
        ...
        <span>
          {estimatedCount} of {totalCount} inputs estimated · {actualCount} confirmed primary
          sources
        </span>
        <button
          type="button"
          onClick={() => window.print()}
          className="rounded border border-slate-300 px-3 py-1 font-semibold text-[#003366] hover:bg-slate-50"
        >
