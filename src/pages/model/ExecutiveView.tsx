...
export default function ExecutiveView({
  ...
  const executiveScenarios = SCENARIOS.filter((scenario) =>
    ['base', 'high_price', 'low_price'].includes(scenario.id),
  );
  ...
  return (
    <div className="space-y-8">
      <FiscalSpaceDiagram
        points={dualRows}
        waccLine={wacc}
        currentRoyaltyRate={currentRoyaltyRate}
      />
        ...
        <div className="mb-3 flex items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Selecting a scenario updates the live fiscal-space chart and executive metrics.
            </p>
          </div>
        </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-700">
          This analysis uses {actualCount} confirmed primary sources and {proxyCount} proxy
          estimates. Proxy estimates are based on publicly documented methodologies.{' '}
          {estimatedCount} inputs require Ministry data to confirm.
        </p>
        <button
          type="button"
          onClick={onAudit}
          className="mt-4 text-sm font-semibold text-[#003366] underline underline-offset-4"
        >
