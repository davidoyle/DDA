...
import { publicPages } from '@/content/siteContent';
...
const searchItems = publicPages.map(page=>({title:page.title,path:page.path,type:page.template==='article'?'Insight':page.topic,description:page.description,haystack:`${page.title} ${page.topic} ${page.description} ${page.source.replace(/[#*|[\]()]/g,' ')}`.toLowerCase()}));
...
export default function Layout(){
 ...
 // Route changes close transient navigation and restore the document viewport.
 // eslint-disable-next-line react-hooks/set-state-in-effect
 useEffect(()=>{setMenu(false);setMega(null);setSearch(false);window.scrollTo(0,0)},[loc.pathname]);
 // Search readiness intentionally resets when the modal opens.
 // eslint-disable-next-line react-hooks/set-state-in-effect
 ...
 const results=searchItems.filter(item=>item.haystack.includes(query.trim().toLowerCase()));
 return <div className="site-shell"><a className="skip-link" href="#main">Skip to main content</a>
  <header className="site-header"><div className="nav-wrap"><Link className="brand" to="/" aria-label="DDA home"><span>DDA</span><i/></Link>
   <nav className="desktop-nav" aria-label="Primary navigation">
    <button className={loc.pathname.startsWith('/what-we-do')?'active':''} onMouseEnter={()=>openWithIntent('work')} onFocus={()=>setMega('work')} onClick={()=>setMega(mega==='work'?null:'work')} aria-expanded={mega==='work'}>What we do <ChevronDown/></button>
    <Link className={loc.pathname.startsWith('/selected-work')?'active':''} to="/selected-work/">Selected work</Link>
    <button className={loc.pathname.startsWith('/insights')?'active':''} onMouseEnter={()=>openWithIntent('think')} onFocus={()=>setMega('think')} onClick={()=>setMega(mega==='think'?null:'think')} aria-expanded={mega==='think'}>Insights <ChevronDown/></button>
    <Link className={loc.pathname.startsWith('/who-we-are')?'active':''} to="/who-we-are/">Who we are</Link><button ref={searchButton} className="icon-button" onClick={()=>setSearch(true)} aria-label="Search"><Search/></button><Link className="nav-contact" to="/contact/">Discuss a decision <ArrowRight/></Link>
   </nav>
