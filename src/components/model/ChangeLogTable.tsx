      ...
      <table className="min-w-full text-sm">
        ...
        <tbody>
          {[...entries]
            ...
            .map((e) => (
              <tr className="border-t" key={`${e.version}-${e.date}`}>
                <td className="p-3">{e.version.toFixed(1)}</td>
                <td className="p-3">{e.date}</td>
                <td className="p-3">{e.modulesAffected.join(', ')}</td>
                <td className="p-3">
                  <strong>{e.description}</strong>
                  <br />
                  <span className="text-xs text-slate-500">
                    {e.reason} — {e.sourceOrAuthority}
                  </span>
                </td>
                <td className="p-3">
