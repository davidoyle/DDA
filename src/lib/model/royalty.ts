...
export function calculateRoyalty(inputs: RoyaltyInputs) {
  ...
  if (spud < frameworkDate && inputs.productionYear < 2027) {
    ...
    return {
      royaltyRate,
      royaltyAmount: netRevenue * royaltyRate,
      framework: 'Existing framework proxy',
      flagsUsed: ['royalty.existingRateTable'],
    };
  }
  if (spud >= frameworkDate && !inputs.capitalRecovered) {
    ...
    return {
      royaltyRate,
      royaltyAmount: netRevenue * royaltyRate,
      framework: 'Transitional capital recovery',
      flagsUsed: [],
    };
  }
  ...
  const ratio = Math.min(
    1,
    Math.max(0, (inputs.currentGasPrice - low) / Math.max(0.01, high - low)),
  );
  const royaltyRate = minRate + ratio * (maxRate - minRate);
  return {
    royaltyRate,
    royaltyAmount: netRevenue * royaltyRate,
    framework: 'Modern price-sensitive framework',
    flagsUsed: ['royalty.thresholdLow', 'royalty.thresholdHigh'],
  };
}

export function royaltySensitivitySweep(
  baseIRR: number | null,
  baseRevenueNPV: number,
  steps = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4],
): DualOutputRow[] {
  const irr = baseIRR ?? 0;
  return steps.map((royaltyRate) => ({
    royaltyRate,
    projectIRR: Math.max(-0.5, irr - (royaltyRate - 0.05) * 0.9),
    provincialRevenueNPV: baseRevenueNPV * (1 + (royaltyRate - 0.05) * 2.2),
    flagsUsed: [
      'royalty.thresholdLow',
      'royalty.thresholdHigh',
      'macro.wacc',
