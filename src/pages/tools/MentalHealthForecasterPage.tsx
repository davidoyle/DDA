...
import {
  healthcareSubSectors,
  mitigationItems,
  pickeringMultipliers,
  rampByYear,
  rtwAdjustmentFactor,
  sectorRates,
} from '@/lib/tools/mental-health-config';
...
export default function MentalHealthForecasterPage() {
  ...
  const projection = useMemo(() => {
    ...
    const rows = Array.from({ length: years }, (_, idx) => {
      const multiplier =
        pickeringMultipliers.base + (rampByYear[idx] ?? rampByYear[rampByYear.length - 1]);
  ...
  const experience = calculateExperienceRatingImpact(
    projection.claimsTotal,
    sectorRates[sector].avgCost,
  );
  ...
  return (
    <div className="diagnostic-theme px-6 lg:px-[8vw] py-12 space-y-6 min-h-screen">
      ...
      <p className="text-[#4a453d]">
        Pickering (2025) and the November 2025 policy rewrite signal sustained claim-growth
        pressure.
      </p>
      ...
      <Card>
        ...
        <CardContent className="grid md:grid-cols-4 gap-4">
          <div>
            <Label>Sector</Label>
            <Select value={sector} onValueChange={(v) => setSector(v as keyof typeof sectorRates)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <ValidatedNumberInput value={headcount} min={0} onValueChange={setHeadcount} />
          </div>
          <div>
          ...
          <div className="flex items-center gap-2">
            <Switch checked={hasRtw} onCheckedChange={setHasRtw} />
            <Label>Documented RTW program</Label>
          </div>
        </CardContent>
      </Card>
      ...
      {sector === 'Healthcare & Social Assistance' && (
        <Alert className="border-rose-500/50">
          <AlertTitle>Presumptive coverage flag</AlertTitle>
          <AlertDescription>
            Nurses and health-care assistants in your sub-sector now have presumptive coverage for
            mental disorder claims. The burden of proof has shifted to the employer.
          </AlertDescription>
        </Alert>
      )}
      ...
      <section className="grid md:grid-cols-3 gap-4">
        <Card>
          <CardHeader>
            <CardTitle>Expected claims ({years}y)</CardTitle>
          </CardHeader>
          <CardContent>
            {projection.claimsTotal.toFixed(1)}
            <div className="mt-2">
              <EvidenceTier tier="MODELLED" />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Expected cost impact</CardTitle>
          </CardHeader>
          <CardContent>
            {fmtMoney(projection.costTotal)}
            <div className="mt-2">
              <EvidenceTier tier="VERIFIED" />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Experience rating impact</CardTitle>
          </CardHeader>
          <CardContent>
            {experience.projectedRateChangePercent.toFixed(1)}%
            <div className="mt-2">
              <EvidenceTier tier="MODELLED" />
            </div>
          </CardContent>
      ...
      {entitlements.canViewActionPlan ? (
        <Card>
          <CardHeader>
            <CardTitle>Mitigation checklist</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {checklist.map((item) => (
              <p key={item.name}>
                • {item.name} ({item.impact}% est.) — {item.source}
              </p>
            ))}
          </CardContent>
        </Card>
        ...
        <FeatureLock
          title="Premium action plan"
          message="Unlock prioritized mitigation checklist and impact estimates."
      ...
      <div className="flex gap-3 items-center">
        <button
          ...
          onClick={() =>
            saveSnapshot('mental-health-forecaster', {
        ...
        {!entitlements.canSaveAndCompare && (
          <span className="text-xs text-[#5b5347]">Pro required for save/compare.</span>
        )}
      </div>
      ...
      <ToolDisclaimer
        toolName="Mental Health Claims Surge Forecaster"
        paramDate="2025-12"
        text="Model output is decision support only and not legal advice."
      />
    </div>
  );
}
