...
export function calculateTax(inputs: TaxInputs) {
  ...
  if (inputs.assetType === 'LNG_FACILITY') {
    ccaRate = Number(resolve(register, 'tax.ccaLNGFacility'));
    flagsUsed.push('tax.ccaLNGFacility');
  }
  const ccaDeduction =
    inputs.undepreciatedCapitalCost * ccaRate * (inputs.yearIndex === 0 ? 0.5 : 1);
  ...
  const obpsCarbonCost =
    (Math.max(0, inputs.facilityEmissions - inputs.benchmarkEmissions) *
      Number(resolve(register, `tax.obps.${obpsYear}`))) /
    1_000_000;
  ...
  return {
    ...
    obpsCarbonCost,
    totalTax,
    effectiveRate: inputs.taxableIncome > 0 ? totalTax / inputs.taxableIncome : 0,
    flagsUsed,
  };
}
