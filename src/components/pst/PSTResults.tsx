...
interface Props {
  ...
  onBehaviorSignal: (signal: {
    viewedRiskFlags?: boolean;
    riskFlagsDwellS?: number;
    viewedAdvocacy?: boolean;
...
export default function PSTResults({ results, segment, onEvent, onBehaviorSignal }: Props) {
  ...
  useEffect(() => {
    const configs: Array<{
      ...
      {
        ref: riskRef,
        eventName: 'risk_flags_viewed',
        payload: { flags_count: results.riskFlags.length },
      },
        ...
        (entries) => {
          entries.forEach((entry) => {
            ...
            if (eventName === 'risk_flags_viewed')
              onBehaviorSignal({ viewedRiskFlags: true, riskFlagsDwellS: dwell });
  ...
  return (
    <section className="px-6 lg:px-[8vw] py-10 space-y-8 print:px-0">
      <div className="flex justify-between items-center print:hidden">
        ...
        <button
          onClick={() =>
            document.getElementById('pst-form')?.scrollIntoView({ behavior: 'smooth' })
          }
      ...
      <article className="card">
        ...
        <ul className="space-y-2 text-sm">
          ...
          {results.aggregateComplianceCost !== undefined && (
            <li>Aggregate compliance estimate: {money(results.aggregateComplianceCost)}</li>
          )}
        </ul>
      </article>
        ...
        <article className="card">
          ...
          <table className="w-full text-sm">
            <tbody>
              <tr>
                <td>Mild drag</td>
                <td>{pct(results.investmentDragMild ?? 0, 3)}</td>
                <td>{money((results.investmentDragMild ?? 0) * results.totalSpend)}</td>
              </tr>
              <tr>
                <td>Central drag</td>
                <td>{pct(results.investmentDragCentral ?? 0, 3)}</td>
                <td>{money((results.investmentDragCentral ?? 0) * results.totalSpend)}</td>
              </tr>
              <tr>
                <td>Severe drag</td>
                <td>{pct(results.investmentDragSevere ?? 0, 3)}</td>
                <td>{money((results.investmentDragSevere ?? 0) * results.totalSpend)}</td>
              </tr>
            </tbody>
          </table>
          <p className="text-sm mt-3">
            The OECD European range (−0.50) is excluded from this analysis — it is not supported by
      ...
      <article ref={advocacyRef}>
        <PSTAdvocacy
          results={results}
          onCtaClick={(ctaId) => onEvent('advocacy_cta_click', { cta_id: ctaId })}
        />
      </article>
      ...
      <CTAPanel
        segment={segment}
        onConsultationClick={() => {
          onBehaviorSignal({ clickedConsultation: true });
          onEvent('consultation_click', { source_panel: 'pst_segment_cta' });
        }}
      />
      ...
      <p className="text-sm text-[#5b5347]">
        Your inputs are used to benchmark this diagnostic against similar firms in your sector. No
        identifying information is stored or shared.
      </p>
      <p className="text-sm text-[#5b5347]">
        This tool does not constitute tax or legal advice. Results are modelled estimates based on
        publicly available data and policy-source assumptions. Parameters last updated:{' '}
        {PARAMETERS_LAST_UPDATED}.
      </p>
    </section>
  );
}
