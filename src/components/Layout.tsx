 ...
 return <div className="site-shell" onClick={event=>{if((event.target as Element).closest('a[href]')){setMenu(false);setMega(null);setSearch(false)}}}><a className="skip-link" href="#main">Skip to main content</a>
  <header className="site-header"><div className="reading-progress" aria-hidden="true"><span style={{transform:`scaleX(${scrollProgress/100})`}}/></div><div className="nav-wrap"><Link className="brand" to="/" aria-label="DDA home"><span>DDA</span><i/></Link>
   <nav className="desktop-nav" aria-label="Primary navigation">
    ...
    <Link aria-current={current('/selected-work/')?'page':undefined} to="/selected-work/">Selected work</Link><button className={current('/insights/')?'is-current':undefined} onMouseEnter={()=>openWithIntent('think')} onFocus={()=>setMega('think')} onClick={()=>setMega(mega==='think'?null:'think')} aria-expanded={mega==='think'} aria-controls="insights-menu">Insights <ChevronDown/></button><Link aria-current={current('/who-we-are/')?'page':undefined} to="/who-we-are/">Who we are</Link><button ref={searchButton} className="icon-button" onClick={()=>setSearch(true)} aria-label="Search"><Search/></button><Link aria-current={current('/contact/')?'page':undefined} className="nav-contact" to="/contact/">Discuss a decision <ArrowRight/></Link>
   </nav>
