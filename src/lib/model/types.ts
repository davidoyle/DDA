...
export interface CashFlowRow {
  ...
  citFederal: number;
  citProvincial: number;
  carbonCost: number;
  atfcf: number;
  cumulativeAtfcf: number;
}
...
export interface ReturnMetrics {
  ...
  governmentTakeUndiscounted: number;
  governmentTakeNPV: number;
  flagsUsed: string[];
  allActual: boolean;
}
export interface DualOutputRow {
  royaltyRate: number;
...
export interface JurisdictionBenchmark {
  id: string;
  name: string;
  govTakeLow: number | null;
  govTakeBase: number | null;
  govTakeHigh: number | null;
  effectiveCITRate: number;
  royaltyType: string;
  carbonPrice: number;
...
export interface ChangeLogEntry {
  ...
  description: string;
  reason: string;
  sourceOrAuthority: string;
  ministryApproval: 'PENDING' | 'APPROVED' | 'NOTED';
}
