...
export default function BCDecarbonizationModelPage() {
  ...
  const handleSaveScenario = (scenario: ScenarioRun) => {
    ...
    setSelectedScenarioIds((current) =>
      [...new Set([...current.slice(-2), scenario.id])].slice(-3),
    );
  };
    ...
    <div className="diagnostic-theme min-h-screen px-6 py-12 lg:px-[8vw]">
      <div className="mx-auto max-w-[1400px] space-y-8">
        <header className="space-y-4">
          ...
          <p className="max-w-4xl text-base text-[#4a453d]">
            Interactive policy simulation for British Columbia&apos;s constrained emissions
            transition, 2026–2030. Explore policy levers, review the baseline case, and compare
            stress-tested scenarios against legal targets.
          </p>
        </header>
        ...
        <Tabs defaultValue="explore" className="space-y-6">
          ...
          <TabsContent value="explore" className="space-y-6">
            <ModelExplorer
              ...
              onChangePhiWeights={setPhiWeights}
        ...
        <ModelInfo />
        <ToolDisclaimer
          toolName="BC Decarbonization Model"
          paramDate="2026-03"
          text="Outputs are scenario estimates derived from official anchors and interpolated constraints; they are not a substitute for official inventory accounting or legal advice."
        />
      </div>
    </div>
  );
