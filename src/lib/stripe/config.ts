export function getStripePublishableKey(): string {
  ...
  if (!key) {
    ...
    if (isDev) {
      console.warn('Missing VITE_STRIPE_PUBLISHABLE_KEY - using test fallback');
...
export const STRIPE_PRICES = {
  ...
  enterprise_monthly:
    import.meta.env.VITE_STRIPE_PRICE_ENTERPRISE_MONTHLY || 'price_enterprise_monthly_placeholder',
  enterprise_annual:
...
export function validatePriceId(priceId: string): boolean {
  const isValid = Object.values(STRIPE_PRICES).includes(
    priceId as (typeof STRIPE_PRICES)[keyof typeof STRIPE_PRICES],
  );
...
export function getPriceIdOrThrow(
  plan: 'pro' | 'enterprise',
  ...
  if (priceId.includes('placeholder')) {
    throw new Error(
      `Stripe price ID not configured for ${plan} ${interval}. Please set environment variable VITE_STRIPE_PRICE_${key.toUpperCase()}`,
    );
  }
