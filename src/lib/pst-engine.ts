...
export function calculatePST(form: PSTFormValues): PSTResults {
  ...
  const serviceInputs = [
    {
      ...
      spend: form.spendAccounting,
      taxableShare: TAXABLE_SHARES.accounting,
    },
    {
      id: 'aeg' as const,
      label: 'Architecture / Engineering / Geoscience',
      spend: form.spendAEG,
      taxableShare: bundlingShare,
    },
    {
      id: 're' as const,
      label: 'Commercial Real Estate',
      spend: form.spendRealEstate,
      taxableShare: TAXABLE_SHARES.re,
    },
    {
      id: 'security' as const,
      label: 'Security Services',
      spend: form.spendSecurity,
      taxableShare: TAXABLE_SHARES.security,
    },
  ];
  ...
  const aggregateComplianceCost = form.employeeCount
    ? form.employeeCount * COMPLIANCE_COST_PER_EMPLOYEE
    : undefined;
  ...
  return {
    ...
    investmentDragCentral: showInvestment
      ? costIncreasePct * INVESTMENT_ELASTICITY.central
      : undefined,
    investmentDragSevere: showInvestment
      ? costIncreasePct * INVESTMENT_ELASTICITY.severe
      : undefined,
...
function buildRiskFlags(form: PSTFormValues, pctOfSpend: number): RiskFlag[] {
  ...
  if (pctOfSpend > 0.05) {
    flags.push({
      ...
      sourceNote:
        'StatsCan Financial Performance NAICS 5412/5413 (2025); IBISWorld NAICS benchmarks',
  ...
  if (form.spendAEG > 0) {
    flags.push({
      ...
      sourceNote:
        'BC MoF Notice 2026-001; ACEC-BC Pre-Budget Submission (2025); Quebec QST post-implementation data',
      evidenceTier: 'MODELLED',
    });
  }
  ...
  flags.push({
    ...
    sourceNote:
      'IMF WP/20/77 (2020); Bank of Canada provincial tax differential studies (2020–2024)',
    evidenceTier: 'MODELLED',
  });
