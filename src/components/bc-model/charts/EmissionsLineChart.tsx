    ...
    ? scenarios[0]?.results.map((point, index) => ({
        ...
        ...Object.fromEntries(
          scenarios.map((scenario) => [
            scenario.id,
            scenario.results[index]?.totalEmissions ?? null,
          ]),
        ),
    ...
    <ChartContainer config={config} className="h-[280px] w-full">
      <LineChart data={chartData} margin={{ left: 8, right: 12, top: 12, bottom: 0 }}>
            ...
            <ChartTooltipContent
              formatter={(value, name) => (
                <>
                  ...
                  <span className="font-mono">{Number(value).toFixed(1)} Mt</span>
                </>
              )}
            />
          }
        ...
        {scenarios ? (
          scenarios.map((scenario, index) => (
            <Line
              ...
              type="monotone"
              dataKey={scenario.id}
              stroke={index === 0 ? '#1f3a5f' : index === 1 ? '#9d4edd' : '#d1603d'}
              strokeWidth={3}
              dot={false}
            />
          ))
        ...
        <ReferenceLine
          ...
          stroke="#d4a03a"
          strokeDasharray="6 4"
          label={{ value: '61.0 Mt legal target', fill: '#6b6255', position: 'insideTopRight' }}
        />
      </LineChart>
    </ChartContainer>
  );
