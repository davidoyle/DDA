  ...
  const rows = useMemo(
    () =>
      selected.map((province) => {
        ...
        const avg3 =
          (rates[province as keyof typeof rates][2024] +
            rates[province as keyof typeof rates][2025] +
            rates[province as keyof typeof rates][2026]) /
          3;
  ...
  return (
    <div className="diagnostic-theme px-6 lg:px-[8vw] py-12 space-y-6 min-h-screen">
      ...
      <div className="card">
        ...
        <div className="flex gap-2 mt-4">
          {([2024, 2025, 2026] as const).map((y) => (
            <button
              key={y}
              className={`btn-secondary ${year === y ? 'border-[#1f3a5f]' : ''}`}
              onClick={() => setYear(y)}
            >
              {y}
            </button>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {provinces.map((p) => (
            <button
              key={p}
              className={`btn-secondary ${selected.includes(p) ? 'border-[#1f3a5f]' : ''}`}
              onClick={() =>
                setSelected((prev) =>
      ...
      <Card>
        ...
        <CardContent>
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.province} className="border-t border-[#ece0cc]">
                  <td>{r.province}</td>
                  <td>{r.currentRate.toFixed(2)}</td>
                  <td>{r.avg3.toFixed(2)}</td>
                  <td>{r.fundedRatio}%</td>
                  <td>{r.surplusPercentPayroll}%</td>
                  <td>{r.unit}</td>
                  <td>{r.annualCost === null ? 'N/A' : `$${r.annualCost.toLocaleString()}`}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {!entitlements.canViewBenchmarks && (
        <FeatureLock
          title="Benchmarking locked"
          message="Unlock cross-province benchmark and repricing risk drivers with Pro licensing."
      ...
      />
      <ToolDisclaimer
        toolName="Multi-Province Comparator"
        paramDate="2026-01"
        text="Washington values are displayed in per-hour units and are not directly commensurate with per-$100 payroll without conversion assumptions."
      />
    </div>
  );
}
