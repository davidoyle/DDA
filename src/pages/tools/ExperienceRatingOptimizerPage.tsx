  ...
  return (
    <div className="diagnostic-theme px-6 lg:px-[8vw] py-12 space-y-6 min-h-screen">
      ...
      <p className="text-[#4a453d]">
        Phase 2 file upload: coming soon. v1 uses manual NEER/retro entry.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>3-year data entry</CardTitle>
        </CardHeader>
          ...
          {[0, 1, 2].map((i) => (
            <div key={i} className="space-y-2">
              ...
              <MoneyInput
                ...
                min={0}
                onValueChange={(value) =>
                  setClaimsCost((prev) => prev.map((v, idx) => (idx === i ? value : v)))
                }
              />
              ...
              <MoneyInput
                ...
                min={0}
                onValueChange={(value) =>
                  setPayroll((prev) => prev.map((v, idx) => (idx === i ? value : v)))
                }
              />
            </div>
          ))}
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          <p>
            Variance: {model.variance.toFixed(2)} ({model.variancePercent.toFixed(1)}%).
          </p>
          {model.variancePercent > appealThresholdPercent && (
            <p className="text-amber-700">
              Appeal flag: variance is above {appealThresholdPercent}% and may justify review.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {entitlements.canCompareScenarios ? (
        <Card>
          <CardHeader>
            <CardTitle>Scenario sensitivity (premium)</CardTitle>
          </CardHeader>
          <CardContent>
            <p>Conservative, Moderate, Aggressive forecast matrix unlocked.</p>
          </CardContent>
        </Card>
      ) : (
        <FeatureLock
          title="Scenario comparison locked"
          message="Unlock multi-scenario forward projections and appeal timing guidance."
      ...
      <button
        ...
        onClick={() =>
          saveSnapshot('experience-rating-optimizer', {
      ...
      />
      <ToolDisclaimer
        toolName="Experience Rating Optimizer"
        paramDate="2026-01"
        text="Scenario output is planning guidance only and does not replace WCB adjudication or appeal advice."
      />
    </div>
  );
}
