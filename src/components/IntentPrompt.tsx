...
export default function IntentPrompt({ onSelect, tone = 'dark' }: Props) {
  ...
  return (
    <section
      className={`px-6 lg:px-[8vw] py-8 border-b print:hidden ${isLight ? 'border-[#d8cdb9]' : 'border-[#F3EFE6]/10'}`}
    >
      <div className="max-w-5xl space-y-4">
        <p
          className={
            isLight ? 'font-mono text-xs uppercase tracking-[0.12em] text-[#6b6255]' : 'eyebrow'
          }
        >
          Before you begin
        </p>
        <h2
          className={isLight ? 'font-heading text-3xl md:text-4xl text-[#131313]' : 'headline-md'}
        >
          What brings you to this diagnostic today?
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {intentOptions.map((option) => (
            ...
            >
              <CardContent className={`py-5 ${isLight ? 'text-[#1f1f1f]' : 'text-[#F3EFE6]/90'}`}>
                {option.label}
              </CardContent>
            </Card>
          ))}
