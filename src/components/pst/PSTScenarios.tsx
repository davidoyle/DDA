  ...
  return (
    <article className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {cards.map((c) => (
          <div key={c.key} className={`card border ${results.responseScenario === c.key ? 'border-[#D4A03A]' : 'border-[#F3EFE6]/20'}`}>
            ...
            <p className="text-sm text-[#F3EFE6]/70 mt-3">{c.mechanism}</p>
          </div>
        ))}
      </div>
