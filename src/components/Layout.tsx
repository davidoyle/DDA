...
export default function Layout(){
 const [menu,setMenu]=useState(false), [mega,setMega]=useState<string|null>(null), [search,setSearch]=useState(false), [query,setQuery]=useState(''), [scrollProgress,setScrollProgress]=useState(0);
 ...
 useEffect(()=>{const update=()=>{const distance=document.documentElement.scrollHeight-window.innerHeight;setScrollProgress(distance>0?Math.min(100,(window.scrollY/distance)*100):0)};update();addEventListener('scroll',update,{passive:true});addEventListener('resize',update);return()=>{removeEventListener('scroll',update);removeEventListener('resize',update)}},[loc.pathname]);
 ...
 return <div className="site-shell" onClick={event=>{if((event.target as Element).closest('a[href]')){setMenu(false);setMega(null);setSearch(false)}}}><a className="skip-link" href="#main">Skip to main content</a>
  <header className="site-header"><div className="reading-progress" aria-hidden="true"><span style={{transform:`scaleX(${scrollProgress/100})`}}/></div><div className="nav-wrap"><Link className="brand" to="/" aria-label="DDA home"><span>DDA</span><i/></Link>
   <nav className="desktop-nav" aria-label="Primary navigation">
    ...
    <button className={current('/insights/')?'is-current':undefined} onMouseEnter={()=>openWithIntent('think')} onFocus={()=>setMega('think')} onClick={()=>setMega(mega==='think'?null:'think')} aria-expanded={mega==='think'} aria-controls="insights-menu">Insights <ChevronDown/></button><Link aria-current={current('/who-we-are/')?'page':undefined} to="/who-we-are/">Who we are</Link><button ref={searchButton} className="icon-button" onClick={()=>setSearch(true)} aria-label="Search"><Search/></button><Link aria-current={current('/contact/')?'page':undefined} className="nav-contact" to="/contact/">Talk to DDA <ArrowRight/></Link>
   </nav>
  ...
  {mega&&<div className="mega" id={mega==='think'?'insights-menu':'what-we-do-menu'} onMouseLeave={()=>setMega(null)} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget))setMega(null)}}><div className="mega-inner">
   ...
   {mega==='work'&&<><div><p className="eyebrow">Capabilities</p>{['Learn the system','Follow the evidence','Test the finding'].map(x=><span className="cap" key={x}>{x}</span>)}</div><div className="mega-service">{services.map(([t,u])=><Link to={u} key={u}>{t}</Link>)}</div><Link className="mega-all" to="/what-we-do/">Explore what we do <ArrowRight/></Link></>}
   {mega==='think'&&<><div><p className="eyebrow">Insights</p><h2>Investigations inside complex systems.</h2></div><div className="mega-service">{insights.map(([t,u])=><Link to={u} key={u}>{t}</Link>)}</div><Link className="mega-all" to="/insights/">Explore insights <ArrowRight/></Link></>}
  </div></div>}
  </header>
  {menu&&<div ref={mobilePanel} className="mobile-panel" role="dialog" aria-modal="true" aria-label="Navigation"><div className="mobile-top"><span className="brand">DDA</span><button onClick={()=>{setMenu(false);menuButton.current?.focus()}} aria-label="Close menu"><X/></button></div><nav><details><summary>What we do <ChevronDown/></summary><div className="mobile-sub"><Link to="/what-we-do/"><b>Explore what we do</b></Link><b>Areas where DDA commonly works</b>{services.map(([t,u])=><Link to={u} key={u}>{t}</Link>)}</div></details><details><summary>Insights <ChevronDown/></summary><div className="mobile-sub"><Link to="/insights/"><b>Explore insights</b></Link>{insights.map(([t,u])=><Link to={u} key={u}>{t}</Link>)}</div></details><Link to="/who-we-are/">Who we are</Link><Link to="/contact/">Talk to DDA</Link></nav></div>}
  {search&&<div ref={searchPanel} className="search-overlay" role="dialog" aria-modal="true" aria-label="Search DDA"><div className="search-box"><div className="search-head"><h2>Search DDA</h2><button onClick={()=>{setSearch(false);searchButton.current?.focus()}} aria-label="Close search"><X/></button></div><label htmlFor="site-search">What are you looking for?</label><div className="search-field"><Search/><input ref={searchInput} id="site-search" value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&results[0])nav(results[0].route)}} placeholder="Search pages and insights"/></div><p aria-live="polite">{query ? `${results.length} result${results.length===1?'':'s'}` : 'Search DDA pages'}</p>{query&&<ul>{results.map(item=><li key={item.route}><Link to={item.route}><span><b>{item.title}</b><small>{item.type.replace('-', ' ')} · {item.description}</small></span><ArrowRight/></Link></li>)}</ul>}{query&&results.length===0&&<p className="empty">No pages match that search. Try a service, topic, or decision.</p>}</div></div>}
  <div id="main" role="main" tabIndex={-1}><Outlet/></div>
  <footer className="site-footer"><div className="footer-lead"><p>Show us what you are working on.</p><Link to="/contact/">Talk to DDA <ArrowRight/></Link><Link to="/what-we-do/">Explore what we do <ArrowRight/></Link></div><div className="footer-grid"><div><div className="brand">DDA</div><p>Deep investigation and analysis for complex public, industrial, regulated, and institutional problems.</p><p>Metro Vancouver, British Columbia</p></div><div><h2>Navigate</h2><Link to="/what-we-do/">What we do</Link><Link to="/insights/">Insights</Link><Link to="/who-we-are/">Who we are</Link><Link to="/contact/">Talk to DDA</Link></div><div><h2>Information</h2><Link to="/privacy/">Privacy</Link><Link to="/legal/">Legal</Link><Link to="/terms/">Terms</Link><Link to="/accessibility/">Accessibility</Link></div></div><div className="copyright">© {new Date().getFullYear()} DDA. All rights reserved.</div></footer>
 </div>
}
