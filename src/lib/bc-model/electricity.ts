...
export function computeElectricityDemandGrowth(controls: PolicyControls, year: number) {
  ...
  const electrificationPush =
    0.45 * controls.buildingsSupport + 0.35 * controls.industrySupport + 0.2 * controls.zevSupport;
  const dualFuelRelief =
    controls.dualFuelPolicy *
    DUAL_FUEL_PEAK_REDUCTION_VS_ALL_ELECTRIC *
    DUAL_FUEL_REGIONAL_DISCOUNT *
    0.05;
  return Math.max(
    0,
    progress * ELECTRICITY_DEMAND_GROWTH_BY_2030 * (0.75 + electrificationPush) - dualFuelRelief,
  );
}

export function computeGridConstraint(
