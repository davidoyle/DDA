...
export default function Layout(){
 ...
 const loc=useLocation(), nav=useNavigate(), menuButton=useRef<HTMLButtonElement>(null), searchInput=useRef<HTMLInputElement>(null);
 useEffect(()=>{setMenu(false);setMega(null);setSearch(false);window.scrollTo(0,0)},[loc.pathname]);
 useEffect(()=>{document.body.style.overflow=(menu||search)?'hidden':''; if(search)setTimeout(()=>searchInput.current?.focus(),0); return()=>{document.body.style.overflow=''}},[menu,search]);
 useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.key==='Escape'){setMega(null);if(search)setSearch(false);if(menu){setMenu(false);menuButton.current?.focus()}}};addEventListener('keydown',key);return()=>removeEventListener('keydown',key)},[menu,search]);
 ...
 return <div className="site-shell"><a className="skip-link" href="#main">Skip to main content</a>
  <header className="site-header"><div className="nav-wrap"><Link className="brand" to="/" aria-label="DDA home"><span>DDA</span><i/></Link>
   <nav className="desktop-nav" aria-label="Primary navigation">
    <button onClick={()=>setMega(mega==='who'?null:'who')} aria-expanded={mega==='who'}>Who we are <ChevronDown/></button>
    <button onClick={()=>setMega(mega==='work'?null:'work')} aria-expanded={mega==='work'}>What we do <ChevronDown/></button>
    <button onClick={()=>setMega(mega==='think'?null:'think')} aria-expanded={mega==='think'}>Our Thinking <ChevronDown/></button>
    <Link to="/selected-work/">Selected work</Link><button className="icon-button" onClick={()=>setSearch(true)} aria-label="Search"><Search/></button><Link className="nav-contact" to="/contact/">Contact <ArrowRight/></Link>
   </nav>
  ...
  {mega&&<div className="mega" onMouseLeave={()=>setMega(null)}><div className="mega-inner">
