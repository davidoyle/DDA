...
const entrypoints = await readFile(
  path.join(root, 'scripts/generate-route-entrypoints.mjs'),
  'utf8',
);
...
for (const [file, route] of pages) {
  ...
  if (!sitemap.includes(`<loc>https://ddanalytics.ca${route}</loc>`))
    fail(`${route}: missing from sitemap`);
...
await access(path.join(root, 'public/dda-insights-page.svg')).catch(() =>
  fail('Insights editorial asset is missing'),
);
if (!styles.includes("url('/dda-insights-page.svg')"))
  fail('Insights editorial asset is not referenced by the public stylesheet');
if (/\.insights-underlay\s*\{[^}]*z-index:\s*-/.test(styles))
  fail('Insights underlay is behind the page background');
if (!websitePage.includes('onBlur={onBlur}') || !websitePage.includes('aria-describedby='))
  fail('Contact fields do not expose post-submit blur validation');
if (!static404.includes('<h1>') || !static404.includes('href="/"'))
  fail('Static 404 is missing its heading or recovery link');
if (entrypoints.includes("writeFile(path.join(distDir, '404.html'), indexHtml)"))
  fail('Build overwrites the dedicated static 404 page');
if (!siteContent.includes('../../.mds/01-home.md?raw'))
  fail('Markdown imports do not resolve from src/content');

if (failures.length) {
  console.error(`Public-site audit failed (${failures.length}):\n- ${failures.join('\n- ')}`);
  process.exit(1);
}
console.log(
  `Public-site audit passed: ${pages.length} routes, headings, links, sitemap entries, static entrypoints, and editorial asset.`,
);
