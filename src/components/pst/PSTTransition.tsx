      ...
      <table className="w-full min-w-[640px] text-sm">
        <thead>
          <tr className="border-b border-[#e3d7c2]">
            ...
            <th className="text-left">Annual PST</th>
            <th className="text-left">Estimated straddle exposure (52%)</th>
          </tr>
        </thead>
        <tbody>
          {transitionRows.map((row) => (
            <tr key={row.service} className="border-b border-[#efe4d1]">
              <td className="py-2">{row.service}</td>
              <td>{money(row.annualPST)}</td>
              <td>{money(row.straddleExposure)}</td>
            </tr>
          ))}
        </tbody>
      </table>
