...
const BCPSTDiagnosticPage = () => {
  ...
  return (
    <div className="diagnostic-theme pt-20 pb-20 min-h-screen">
      <section className="px-6 lg:px-[8vw] py-10 border-b border-[#e3d7c2]">
        <p className="eyebrow">Policy Tools</p>
        <h1 className="headline-lg max-w-4xl">BC PST Impact Diagnostic</h1>
        <p className="text-[#4a453d] text-lg max-w-3xl mt-4">
          Quantify the cost of BC&apos;s October 2026 PST expansion on professional services.
        </p>
      </section>

      <section className="px-6 lg:px-[8vw] py-10 space-y-8 border-b border-[#ece0cc]">
        <div className="grid md:grid-cols-2 gap-4">
          <label className="space-y-2">
            <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">Industry sector</span>
            <select className="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3" value={sector} onChange={(e) => setSector(e.target.value as Sector)}>
              {Object.entries(SECTOR_DATA).map(([key, value]) => <option key={key} value={key} className="text-[#0B3C43]">{value.label}</option>)}
            </select>
          </label>
          <label className="space-y-2">
            <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">Firm size</span>
            <select className="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3" value={firmSize} onChange={(e) => setFirmSize(e.target.value as FirmSize)}>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[
            ...
            <label key={label} className="space-y-2">
              <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">{label}</span>
              <MoneyInput value={value} min={0} onValueChange={setter} inputClassName="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3" />
            </label>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <label className="space-y-2">
            <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">AEG bundling assumption</span>
            <select className="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3" value={bundling} onChange={(e) => setBundling(e.target.value as BundlingMode)}>
          ...
          <label className="space-y-2">
            <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">Behavioural response scenario</span>
            <select className="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3" value={response} onChange={(e) => setResponse(e.target.value as ResponseScenario)}>
      ...
      {ran && calc.totalSpend > 0 ? (
        <section className="px-6 lg:px-[8vw] py-10 space-y-6">
          <div className="grid md:grid-cols-3 gap-4">
            <article className="card"><p className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">Annual PST cost</p><p className="font-heading text-3xl text-[#9A6A28] mt-2">{money(calc.totalPST)}</p></article>
            <article className="card"><p className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">% of services spend</p><p className="font-heading text-3xl mt-2">{calc.pctOfSpend.toFixed(1)}%</p></article>
            <article className="card"><p className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">Year 1 total impact</p><p className="font-heading text-3xl text-[#9A6A28] mt-2">{money(calc.year1Impact)}</p></article>
          </div>

          <article className="card overflow-x-auto">
            <h2 className="font-heading text-2xl mb-4">PST Cost Breakdown by Service</h2>
            <table className="w-full min-w-[760px] text-sm">
              <thead><tr className="border-b border-[#d8cdb9]"><th className="text-left py-2">Service</th><th className="text-left py-2">Spend</th><th className="text-left py-2">Effective Rate</th><th className="text-left py-2">PST Cost</th><th className="text-left py-2">Net Cost</th></tr></thead>
              <tbody>
                {calc.rows.map((r) => (
                  <tr key={r.id} className="border-b border-[#ece0cc]"><td className="py-2">{r.label}</td><td>{money(r.spend)}</td><td>{r.effRate.toFixed(2)}%</td><td>{money(r.pst)}</td><td>{money(r.netCost)}</td></tr>
                ))}
              </tbody>
            </table>
          </article>
          ...
          <article className="card">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {(['low', 'medium', 'high'] as ResponseScenario[]).map((key) => {
                ...
                return <div key={key} className={`rounded-lg border p-3 ${response === key ? 'border-[#1f3a5f]' : 'border-[#d8cdb9]'}`}><p className="font-mono text-xs uppercase">{key} response</p><p className="font-heading text-2xl mt-2">{money(remaining)}</p><p className="text-xs text-[#5b5347] mt-1">{(erosion * 100).toFixed(1)}% erosion</p></div>;
              })}
            </div>
          </article>
          ...
          <article className="card">
            ...
            <div className="space-y-2">
              {calc.risks.map((risk) => <div key={risk.title} className="rounded-lg border border-[#d8cdb9] px-3 py-2"><p className="font-semibold">{risk.title}</p><p className="text-sm text-[#5b5347]">{risk.text}</p></div>)}
            </div>
          </article>
        </section>
      ) : null}
