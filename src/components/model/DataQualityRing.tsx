...
export default function DataQualityRing({ actualCount, totalCount, size = 56, onClick, label = 'Open data quality drawer' }: { actualCount: number; totalCount: number; size?: number; onClick?: () => void; label?: string }) {
  ...
  return <button type="button" aria-label={label} onClick={onClick} className={cn('inline-flex items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-[#003366] focus:ring-offset-2', onClick && 'cursor-pointer')} style={{ width: size, height: size }}>
