...
const normal = (mean: number, sd: number) =>
  mean + sd * Math.sqrt(-2 * Math.log(Math.random())) * Math.cos(2 * Math.PI * Math.random());
...
const quantile = (arr: number[], q: number) =>
  arr[Math.min(arr.length - 1, Math.max(0, Math.floor(q * (arr.length - 1))))] ?? 0;
...
export function runMonteCarlo({
  ...
  for (let i = 0; i < iterations; i += 1) {
    ...
    const capex = normal(1, 0.12);
    const opex = normal(1, 0.08);
    const fx = normal(1, 0.05);
  ...
  return {
    iterations,
    expectedNPV: npvs.reduce((a, b) => a + b, 0) / iterations,
    p10NPV: quantile(npvs, 0.1),
    p90NPV: quantile(npvs, 0.9),
    probabilityNegativeNPV: npvs.filter((v) => v < 0).length / iterations,
    irrDistribution: irrs.slice(0, 200),
    revenueVaR: baseGovRevenueNPV - quantile(revenues, 0.05),
    flagsUsed: [...P1_FLAG_IDS, ...P2_FLAG_IDS],
  };
}
