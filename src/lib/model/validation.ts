...
export function validateOutput(
  register: AssumptionRegister = BASE_ASSUMPTIONS,
  flagsUsed: string[] = [],
): ValidationResult {
  const unique = [...new Set(flagsUsed)].filter(
    (id) => register[id] || BASE_ASSUMPTIONS[id] || id === 'royalty.existingRateTable',
  );
  ...
  const warningLevel: WarningLevel =
    unique.length === 0 ? 'NONE' : p1FlagsUsed.length > 0 ? 'ESTIMATED' : 'CAUTION';
  return {
    flagsUsed: unique,
    p1FlagsUsed,
    p2FlagsUsed,
    allActual: unique.length === 0,
    warningLevel,
  };
}
