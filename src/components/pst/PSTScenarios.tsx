...
const cards = [
  {
    key: 'low',
    label: 'Low response',
    mechanism: 'Limited contract restructuring and weak supplier substitution.',
  },
  {
    key: 'medium',
    label: 'Medium response',
    mechanism: 'Moderate renegotiation and procurement shifts by firms.',
  },
  {
    key: 'high',
    label: 'High response',
    mechanism: 'Strong restructuring, sourcing shifts, and service redesign.',
  },
] as const;
    ...
    <article className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {cards.map((c) => (
          <div
            key={c.key}
            className={`card border ${results.responseScenario === c.key ? 'border-[#D4A03A]' : 'border-[#d8cdb9]'}`}
          >
      ...
      <Alert className="border-[#D4A03A]">
        ...
        <AlertDescription>
          Behavioural erosion can reduce near-term liability, but also weakens long-run BC tax base
          competitiveness and increases probability of policy reversal pressure.
        </AlertDescription>
      </Alert>
    </article>
  );
