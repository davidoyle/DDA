...
const SECTOR_DATA: Record<
  Sector,
  { label: string; passthroughSmall: number; passthroughMid: number; passthroughLarge: number }
> = {
  construction: {
    label: 'Construction & Real Estate Development',
    passthroughLarge: 0.9,
    passthroughMid: 0.75,
    passthroughSmall: 0.52,
  },
  mining: {
    label: 'Mining & Natural Resources',
    passthroughLarge: 0.92,
    passthroughMid: 0.78,
    passthroughSmall: 0.55,
  },
  tech: {
    label: 'Technology & Professional Services',
    passthroughLarge: 0.65,
    passthroughMid: 0.55,
    passthroughSmall: 0.4,
  },
  retail: { label: 'Retail', passthroughLarge: 0.6, passthroughMid: 0.5, passthroughSmall: 0.35 },
  manufacturing: {
    label: 'Manufacturing',
    passthroughLarge: 0.7,
    passthroughMid: 0.6,
    passthroughSmall: 0.42,
  },
  finance: {
    label: 'Finance & Insurance',
    passthroughLarge: 0.8,
    passthroughMid: 0.68,
    passthroughSmall: 0.48,
  },
  other: {
    label: 'Other Services',
    passthroughLarge: 0.52,
    passthroughMid: 0.44,
    passthroughSmall: 0.32,
  },
};
...
const BCPSTDiagnosticPage = () => {
  ...
  const calc = useMemo(() => {
    const passThrough =
      firmSize === 'small'
        ? SECTOR_DATA[sector].passthroughSmall
        : firmSize === 'mid'
          ? SECTOR_DATA[sector].passthroughMid
          : SECTOR_DATA[sector].passthroughLarge;
    ...
    if (pctOfSpend > 5) {
      risks.push({
        ...
        text: 'PST exceeds 5% of professional services spend.',
      });
    } else if (pctOfSpend > 2.5) {
      risks.push({
        level: 'medium',
        title: 'Moderate margin pressure',
        text: 'PST is 2.5–5% of professional services spend.',
      });
    } else {
      risks.push({
        level: 'low',
        title: 'Lower direct exposure',
        text: 'PST is under 2.5% of professional services spend.',
      });
    }
      ...
      <section className="px-6 lg:px-[8vw] py-10 space-y-8 border-b border-[#ece0cc]">
        <div className="grid md:grid-cols-2 gap-4">
          <label className="space-y-2">
            <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">
              Industry sector
            </span>
            <select
            ...
            >
              {Object.entries(SECTOR_DATA).map(([key, value]) => (
                <option key={key} value={key} className="text-[#0B3C43]">
                  {value.label}
                </option>
              ))}
            </select>
          ...
          <label className="space-y-2">
            ...
            >
              <option value="small" className="text-[#0B3C43]">
                Small — under $2M revenue
              </option>
              <option value="mid" className="text-[#0B3C43]">
                Mid-size — $2M–$20M revenue
              </option>
              <option value="large" className="text-[#0B3C43]">
                Large — over $20M revenue
              </option>
            </select>
          </label>
        </div>
          ...
          {[
            ...
            <label key={label} className="space-y-2">
              ...
              <MoneyInput
                ...
                inputClassName="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3"
              />
            </label>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <label className="space-y-2">
            <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">
              AEG bundling assumption
            </span>
            <select
              className="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3"
              value={bundling}
              onChange={(e) => setBundling(e.target.value as BundlingMode)}
            >
              <option value="base" className="text-[#0B3C43]">
                Base case — 60–70% bundling persistence
              </option>
              <option value="low" className="text-[#0B3C43]">
                Low — full unbundling
              </option>
              <option value="high" className="text-[#0B3C43]">
                High — full bundling persistence
              </option>
            </select>
          </label>
          <label className="space-y-2">
            <span className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">
              Behavioural response scenario
            </span>
            <select
              className="w-full rounded-lg bg-white border border-[#d8cdb9] px-4 py-3"
              value={response}
              onChange={(e) => setResponse(e.target.value as ResponseScenario)}
            >
              <option value="low" className="text-[#0B3C43]">
                Low
              </option>
              <option value="medium" className="text-[#0B3C43]">
                Medium
              </option>
              <option value="high" className="text-[#0B3C43]">
                High
        ...
        {ran && calc.totalSpend === 0 ? (
          <p className="text-red-300 text-sm">Please enter at least one service spend amount.</p>
        ) : null}
      </section>
        ...
        <section className="px-6 lg:px-[8vw] py-10 space-y-6">
          <div className="grid md:grid-cols-3 gap-4">
            <article className="card">
              <p className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">
                Annual PST cost
              </p>
              <p className="font-heading text-3xl text-[#9A6A28] mt-2">{money(calc.totalPST)}</p>
            </article>
            <article className="card">
              <p className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">
                % of services spend
              </p>
              <p className="font-heading text-3xl mt-2">{calc.pctOfSpend.toFixed(1)}%</p>
            </article>
            <article className="card">
              <p className="font-mono text-xs uppercase tracking-[0.1em] text-[#5b5347]">
                Year 1 total impact
              </p>
              <p className="font-heading text-3xl text-[#9A6A28] mt-2">{money(calc.year1Impact)}</p>
            </article>
          </div>
            ...
            <table className="w-full min-w-[760px] text-sm">
              <thead>
                <tr className="border-b border-[#d8cdb9]">
                  ...
                  <th className="text-left py-2">PST Cost</th>
                  <th className="text-left py-2">Net Cost</th>
                </tr>
              </thead>
              <tbody>
                {calc.rows.map((r) => (
                  <tr key={r.id} className="border-b border-[#ece0cc]">
                    <td className="py-2">{r.label}</td>
                    <td>{money(r.spend)}</td>
                    <td>{r.effRate.toFixed(2)}%</td>
                    <td>{money(r.pst)}</td>
                    <td>{money(r.netCost)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {(['low', 'medium', 'high'] as ResponseScenario[]).map((key) => {
                ...
                return (
                  <div
                    key={key}
                    className={`rounded-lg border p-3 ${response === key ? 'border-[#1f3a5f]' : 'border-[#d8cdb9]'}`}
                  >
                    <p className="font-mono text-xs uppercase">{key} response</p>
                    <p className="font-heading text-2xl mt-2">{money(remaining)}</p>
                    <p className="text-xs text-[#5b5347] mt-1">
                      {(erosion * 100).toFixed(1)}% erosion
                    </p>
                  </div>
                );
              })}
          ...
          <article className="card">
            ...
            <div className="space-y-2">
              {calc.risks.map((risk) => (
                <div key={risk.title} className="rounded-lg border border-[#d8cdb9] px-3 py-2">
                  <p className="font-semibold">{risk.title}</p>
                  <p className="text-sm text-[#5b5347]">{risk.text}</p>
                </div>
              ))}
            </div>
          </article>
