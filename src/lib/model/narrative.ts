...
export function generateNarrative(
  ...
  const govTake =
    metrics.governmentTakeUndiscounted > 0
      ? metrics.governmentTakeNPV / metrics.governmentTakeUndiscounted
      : 0;
  ...
  if (viableRows.length) {
    ...
    const current =
      dualOutputRows.find((row) => Math.abs(row.royaltyRate - 0.05) < 0.001)?.royaltyRate ?? 0.05;
    const position =
      current >= min && current <= max ? 'falls within' : current > max ? 'exceeds' : 'is below';
