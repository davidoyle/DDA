...
const PSTFormSchema = z
  ...
  .refine(
    (data) => data.spendAccounting + data.spendAEG + data.spendRealEstate + data.spendSecurity > 0,
    {
...
const tooltipMap: Record<string, string> = {
  ...
  'Architecture / Engineering / Geoscience':
    'Legislated rate is 2.1% (30% taxable base). This tool uses 2.94% (base case) because most integrated project engagements do not separately itemise taxable components.',
...
export default function PSTForm({ onSubmit, onToggleUsed }: PSTFormProps) {
  ...
  return (
    <section
      id="pst-form"
      className="px-6 lg:px-[8vw] py-10 space-y-8 border-b border-[#d8cdb9] print:hidden"
    >
      <form onSubmit={form.handleSubmit(handleValidSubmit)} className="space-y-8">
        <div className="space-y-4">
          ...
          <label className="space-y-2 block max-w-xl">
            ...
            <input
              ...
              type="number"
              min={1}
              placeholder="Optional — used for aggregate compliance estimate."
              ...
              <label className="space-y-2" key={field.name}>
                ...
                <div className="relative">
                  ...
                  <input
                    type="number"
                    min={
                      field.name === 'spendSecurity'
                        ? 0
                        : field.name === 'spendRealEstate'
                          ? 5000
                          : field.name === 'spendAccounting'
                            ? 10000
                            : 50000
                    }
                    max={
                    ...
                    className="w-full rounded-md border bg-transparent pl-8 pr-3 py-2"
            ...
            <AccordionContent>
              <div className="space-y-4 pt-2">
                <label className="space-y-2 block">
                  ...
                  <select
                    className="w-full rounded-md border bg-transparent px-3 py-2"
                    {...form.register('bundlingScenario')}
                  >
                    <option value="low">Low (30%)</option>
                    <option value="base">Base (42%)</option>
                    <option value="high">High (52%)</option>
                  </select>
                  <p className="text-sm text-[#5c5548]">
                    Changes the taxable share applied to AEG spend based on observed bundling
                    persistence.
                  </p>
                </label>
                <label className="space-y-2 block">
                  ...
                  <select
                    className="w-full rounded-md border bg-transparent px-3 py-2"
                    {...form.register('responseScenario')}
                  >
                  ...
                  <p className="text-sm text-[#5c5548]">
                    Highlights the scenario card to match your strategic planning assumption.
                  </p>
                </label>
                <label className="space-y-2 block">
                  ...
                  <input
                    ...
                    step="0.01"
                    className="w-full rounded-md border bg-transparent px-3 py-2"
                    {...form.register('passthroughOverride', {
                      setValueAs: (v) => (v === '' ? undefined : Number(v)),
                    })}
                  />
                  <p className="text-sm text-[#5c5548]">
                    Overrides sector-size pass-through defaults sourced from StatsCan I-O and OECD
                    elasticity synthesis.
                  </p>
                </label>
              </div>
            </AccordionContent>
        ...
        {form.formState.errors.root?.message && (
          <p className="text-red-300">{form.formState.errors.root.message}</p>
        )}
        <button type="submit" disabled={!hasSpend} className="btn-primary disabled:opacity-50">
          Run diagnostic
        </button>
        {isDemoMode ? (
          <p className="text-sm text-[#6b6255]">
            Demo restrictions active: sector and firm size are locked to demo defaults.
          </p>
        ) : null}
      </form>
    </section>
